...
export default function MyEvents() {
  ...
  const filtered = EVENTS
    ...
    .sort((a, b) => {
      if (sort === 'Date (soonest)')  return new Date(a.dateISO).getTime() - new Date(b.dateISO).getTime();
      if (sort === 'Date (latest)')   return new Date(b.dateISO).getTime() - new Date(a.dateISO).getTime();
  ...
  return (
    <div style={{ padding: 32, background: colors.white, minHeight: '100%' }}>
      ...
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
        <h1 style={{ fontSize: 22, fontWeight: 700, color: themeColors.gray900, margin: 0 }}>
          My Events
        </h1>
      </div>

      {/* Tabs */}
      <div style={{ display: 'flex', gap: 24, marginTop: 16, borderBottom: `1px solid ${themeColors.gray200}` }}>
      ...
      <div style={{ display: 'flex', gap: 10, marginTop: 16, alignItems: 'center' }}>
        ...
        <div ref={categoryRef} style={{ position: 'relative' }}>
          <button
            ...
            style={dropdownButtonStyle(themeColors, colors, categoryOpen, TEAL, selectedCategories.length > 0)}
          >
            <svg width={13} height={13} viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth={2} style={{ marginRight: 5 }}>
        ...
        <div style={{ display: 'flex', gap: 6, marginTop: 10, flexWrap: 'wrap' }}>
          {selectedCategories.map((cat) => {
            ...
            return (
              ...
              >
                {cat}
                <svg width={9} height={9} viewBox="0 0 24 24" fill="none" stroke={s.text} strokeWidth={3}>
      ...
      {filtered.length === 0 ? (
        ...
        >
          {filtered.map((event) => (
            <EventCard key={event.id} event={event} themeColors={themeColors} colors={colors} teal={TEAL} />
          ))}
        </div>
      )}
    </div>
...
function DropdownMenu({ children, minWidth = 160 }: { children: React.ReactNode; minWidth?: number }) {
  return (
    <div
      style={{
        position: 'absolute',
...
function DropdownItem({
  ...
  return (
    ...
    >
      {label}
      {selected && (
        <svg width={13} height={13} viewBox="0 0 24 24" fill="none" stroke={teal} strokeWidth={2.5}>
          <polyline points="20 6 9 17 4 12" />
        </svg>
      )}
    </div>
      ...
      <div style={{ padding: '14px 14px 12px' }}>
        ...
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 6 }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
            <span style={{ fontSize: 11, color: themeColors.gray700, fontWeight: 500, letterSpacing: '0.04em' }}>
              {event.displayDate}
            </span>
            <span
              style={{
          ...
          {event.isStarred && (
            <svg width={15} height={15} viewBox="0 0 24 24" fill="#F6C744" stroke="#F6C744" strokeWidth={1.5}>
        ...
        {/* Title */}
        <div style={{ fontWeight: 700, fontSize: 15, color: themeColors.gray900, lineHeight: 1.3 }}>
          {event.title}
        </div>
        ...
        {(event.timeRange || event.location) && (
          <div
            style={{ display: 'flex', alignItems: 'center', gap: 6, marginTop: 6, fontSize: 12, color: themeColors.gray700 }}
          >
            {event.timeRange && (
              <>
