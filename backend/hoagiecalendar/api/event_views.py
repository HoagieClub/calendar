...
class EventView(APIView):
	def get(self, request) -> Response:
		...
		if start_time is not None:
			dt_start = _parse_query_datetime(start_time)
			if dt_start is None:
				return Response(
					{"detail": "Invalid 'start_time' datetime."},
					status=status.HTTP_400_BAD_REQUEST,
				)
			qs = qs.filter(end__gte=dt_start)
		if end_time is not None:
			dt_end = _parse_query_datetime(end_time)
			if dt_end is None:
				return Response(
					{"detail": "Invalid 'end_time' datetime."},
					status=status.HTTP_400_BAD_REQUEST,
				)
